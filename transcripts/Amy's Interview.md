# Informal in class Interview
## Key points and Tree
**Topic:** User research interview about event attendance habits (anime/cosplay conventions and similar events)

**Key points:**

- **Most recent event:** Fan Expo (anime/cosplay convention) in August, attended with friends
- **Primary motivation:** Buying merchandise from artists — conventions are seen as a concentrated place to find artists selling their work that aren't available in daily life
- **Secondary motivation:** Socializing — cosplaying together with friends, meeting new people who share interests, and taking photos together
- **Attendance pattern:** Usually goes with existing friends, but open to meeting new people at the event (especially other cosplayers)
- **Decision factors, ranked:**
    1. **Location** — strong preference for downtown/convenient transit access (mentions "just take Union"); events in Waterloo are seen as too far (3-4 hour drive) and are a deterrent
    2. **Price** — cost sensitivity, including choosing to attend on weekdays when ticket prices are cheaper than weekends
- **Most memorable aspects:** Capturing photos of the moment and socializing with friends
- **Future intent:** Plans to keep attending this type of event annually (frames it as a yearly tradition), driven by the same location/price factors
- **Other events attended:** Food festivals — motivated by trying different foods and casual hangout time with friends
- **Discovery method:** Social media (Instagram) — event organizers/pages post about upcoming weekend events
- **Decision to attend:** Largely driven by whether friends are interested in going together, not a solo decision

**Overall takeaway:** This person's event attendance is driven by two main motivators (shopping/merch access and social bonding with friends) and gated primarily by two practical constraints (location/convenience and price), with social media as the main discovery channel and friend group interest as the deciding factor.


### Needs

**Need: Access to independent artists/merch not available elsewhere**

> "There's no then much. Artists to sell their products, so that is a place I can find those artists and find them."
> 

**Need: Convenient transit/location**

> "I feel like that's pretty convenient for me, just take, and just in Union, so it's pretty convenience."
> 

**Need: Companionship to attend**

> "it's usually based on if your friends are interested"
> 

### Pains

**Pain: Long travel distance discourages attendance**

> "there are some events that maybe in Waterloo, and I will find that there's too far away. It's three to four Drive"
> 

**Pain: High cost, especially on weekends**

> "last night I feel like the price is too expensive, and also the price depends on which day your tend to event"
> 

**Pain: Limited chances to meet people with shared interests in daily life**

> "sometimes I won't have a chance to meet all that kind of enhance" *(likely means "enthusiasts" — transcription error, but the sentiment of limited access to like-minded people is clear from context)*
> 

### Desires

**Desire: Buying merchandise**

> "my man idea is to buy some merch from that"
> 

**Desire: Socializing/making friends through shared cosplay**

> "made friends, because yeah, sometimes, where it's a cosplay together and meet friends and substance photos" *(likely "take" not "substance" — transcription artifact)*
> 

**Desire: Capturing memories through photos**

> "It's like some pictures to capture that moment and also talk to friends"
> 

**Desire: Trying variety/novelty (extends beyond conventions to food festivals)**

> "I want to try different foods, and also that is a really good place to hang out with friends"
> 

**Desire: Cost-saving behavior (choosing cheaper days)**

> "sometimes I would choose go there in work day"
> 

**Desire: Recurring/ritual attendance**

> "usually I would hand every and my explore into it in every year" *(likely means "attend" — transcription error, but the intent of annual/recurring attendance is clear)*
> 

---

## Flagged inferences (no direct quote to support them)

- **Inferred need: Social validation/belonging through a niche community.** The user's answers keep circling back to friendship and shared identity (cosplaying together, meeting "enhance"/enthusiasts), suggesting community belonging is a deeper driver than just "making friends" — but this reading is my interpretation, not something they stated outright.
- **Inferred pain: Difficulty finding people with niche interests (anime/cosplay) in everyday life.** The user implies this by contrasting conventions with "daily life," but never explicitly says finding people is hard — I'm reading that into the merch/artist comment being repurposed toward a social need.
- **Inferred desire: Low-effort discovery.** The user's reliance on Instagram/social media posts suggests they want passive discovery (things surfaced to them) rather than actively searching — this is inferred from behavior, not stated as a preference.


### Tree
```
OUTCOME: Increase repeat attendance & spend at niche fan events (conventions, festivals)
│
├── OPPORTUNITY: Access to independent artists/merch not available elsewhere
│   "There's no then much. Artists to sell their products, so that is a place I can find those artists and find them."
│   ├── Pre-event artist/vendor directory or map, so attendees can plan purchases in advance
│   └── "Notify me" alerts when a followed artist confirms attendance at an upcoming event
│
├── OPPORTUNITY: Long travel distance discourages attendance
│   "there are some events that maybe in Waterloo, and I will find that there's too far away. It's three to four Drive"
│   ├── Filter/sort events by distance or transit time from home base
│   └── Group carpool or charter bus coordination for out-of-city events
│
├── OPPORTUNITY: High cost, especially on weekends
│   "last night I feel like the price is too expensive, and also the price depends on which day your tend to event"
│   ├── Weekday discount ticket tiers, surfaced prominently
│   └── Price-drop / cheap-day alerts pushed to users before they buy
│
├── OPPORTUNITY: Convenient transit/location as a deciding factor
│   "I feel like that's pretty convenient for me, just take, and just in Union, so it's pretty convenience."
│   └── Highlight "transit-accessible" badge on event listings (e.g., near Union Station)
│
├── OPPORTUNITY: Companionship needed to attend
│   "it's usually based on if your friends are interested"
│   ├── "Find a squad" feature — invite/RSVP flow to gauge friend interest before buying tickets
│   └── Group ticket bundles/discounts to incentivize going together
│
├── OPPORTUNITY: Socializing/making friends through shared cosplay
│   "made friends, because yeah, sometimes, where it's a cosplay together and meet friends and [take] photos"
│   ├── In-app cosplay meetup boards (match by character/fandom at a specific event)
│   └── Shared photo album/tagging feature for event-day photos
│
├── OPPORTUNITY: Capturing memories through photos
│   "It's like some pictures to capture that moment and also talk to friends."
│   └── Auto-generated event recap/photo montage feature post-event
│
├── OPPORTUNITY: Recurring/ritual attendance
│   "usually I would [attend] every ... explore into it in every year"
│   └── Annual pass or loyalty program for repeat attendees of the same event
│
└── OPPORTUNITY [assumption — no quote yet]: Difficulty finding people with niche interests in daily life
└── Year-round community/discussion space (not just event-day), to sustain the relationships formed at events
```

### Transcript
So, First off, can you walk me through the last time that you attended an event in the city?

Is it like and in the explore turns us?

Yeah, for sure.

Was it Fan Expo?

Yeah, August.

And Cosplay.

So, That's the event.

I mostly often to attempt, and I feel like most of time My man idea is to buy some merch from that.

Okay, So when you went to that event, your main goal was to buy more.

Yes, so what made you decide to go to this type of event?

Feel like daily life.

There's no then much.

Artists to sell their products, so that is a place I can find those artists and find them.

So, So for this type of event, it would specifically just be to go by.

The merch also made friends, because yeah, sometimes, where it's a cosplay together and meet friends and substance photos.

So, do you usually go to these checks events with friends already, or do you like to go find food?

So most likely I will go with friends and maybe at a time if I find someone like to listen I.T cosplay and I will also like meet new friends as well.

So, what is the most important factors about why you liter technique or what makes you an attending Because I like this?

And we're like, very simple reason.

And Okay.

So, like, Do consider things like location, time, money?

Yeah, that would be Them.

I think location is both things I will put in first because there are some events that maybe in Waterloo, and I will find that there's too far away.

It's three to four Drive, and I was at that.

But by facing downtown.

I feel like that's pretty convenient for me, just take, and just in Union, so it's pretty convenience.

And also prices are Vegas reason, because last night I feel like the price is too expensive, and also the price depends on which day your tend to event.

If it's like, Um, work day, it would be much cheaper than we can.

So, sometimes I would choose go there in work day.

Okay, so what would?

What's the most memorable thing about these types of events?

It's like some pictures to capture that moment and also talk to friends.

Yeah, and I feel like, um, this is also a really good way to meet new friends as well, and sometimes I won't have a chance to meet all that kind of enhance.

Okay, so would you attend more events like this?

Yeah, for sure.

Like, usually I would hand every and my explore into it in every year, and I guess the same factors that you mentioned before influencer decision to go.

I'm sorry, like you were saying, how like the location is really important to those who would influence your decisions.

It also surprised.

I think that is most important.

Okay, I've run to the questions on the script, so I'll just keep it sprinkler.

I'll just keep asking random ones.

So, what other types of events do you like to go to outside of Antioch Conventions?

Like food festival.

That would be also one because I want to try different foods, and also that is a really good place to hang out with friends, you can just talk anything to help everyone as well.

How do you usually find these type of statements?

While Instagram or selfish shoe, they would have like a post that just say that, oh, this weekend.

And So, how do you decide to actually go to those?

That's my friends.

If you want to go with me, so I need someone with me, so it's usually based on if your friends are interested.
